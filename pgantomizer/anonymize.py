...
def null_anonymize(column, pk_name):
    return "NULL"


def replace_with_column_and_pk(column, pk_name):
    return f"'{column}_' || {pk_name}"


def aggregate_length(column, pk_name):
    return f"length({column})"


def x_out(column, pk_name):
    """Replace non-whitespace chars with 'x' and keep the whitespace."""
    return f"regexp_replace({column}, '\S', 'x', 'g')"


def anonymize_email(column, pk_name):
    """Turns 'william.riker@gmail.com' into '1@example.com' if 1 is the ID of the record.'"""
    return f"{pk_name} || '@example.com'"


def md5_anonymize(column, pk_name):
    return f"MD5({column})"
...
ANONYMIZE_DATA_TYPE = {
    ...
    "character varying": replace_with_column_and_pk,
    "text": replace_with_column_and_pk,
...
def get_table_pk_name(schema, table):
    return (
        schema[table].get("pk", DEFAULT_PK_COLUMN_NAME)
        if schema[table]
        else DEFAULT_PK_COLUMN_NAME
    )
...
def prepare_column_for_anonymization(conn, cursor, table, column, data_type):
    ...
    if data_type == "character varying":
        ...
        cursor.execute(
            "ALTER TABLE {table} ALTER COLUMN {column} TYPE varchar(250);".format(
...
def check_schema(cursor, schema, db_args):
    for table in schema:
        ...
        if "truncate" in schema[table]:
            if schema[table]["truncate"] in [True, "cascade"]:
                continue
            else:
                raise InvalidAnonymizationSchemaError(
                    "Invalid value for `truncate`: {}".format(schema[table]["truncate"])
                )
        ...
        try:
            cursor.execute(
                "SELECT {columns} FROM {table} LIMIT 1;".format(
                    columns='"{}"'.format('", "'.join(columns_to_validate)), table=table
                )
            )
...
def get_column_update(schema, table, column, data_type):
    ...
    if column == get_table_pk_name(schema, table) or (
        ...
        if custom_rule and isinstance(custom_rule, dict) and "value" in custom_rule:
        ...
        anonymization = (
            CUSTOM_ANONYMIZATION_RULES[custom_rule]
            if custom_rule
            else ANONYMIZE_DATA_TYPE[data_type]
        )
        return "{column} = {value}".format(
            column=column,
            value=anonymization(column, get_table_pk_name(schema, table))
            if callable(anonymization)
            else anonymization,
        )
    else:
        raise MissingAnonymizationRuleError(
            'No rule to anonymize type "{}" for column "{}"'.format(data_type, column)
        )
...
def anonymize_table(conn, cursor, schema, table, disable_schema_changes):
    ...
    if schema[table] and "truncate" in schema[table]:
        cascade = ""
    ...
    if len(column_updates) > 0:
        ...
        logging.debug(
            "Running UPDATE on {} for columns {} ...".format(
                table, ", ".join(updated_column_names)
            )
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Load data from a Postgres dump to a specified instance "
    ...
    parser.add_argument(
        ...
        "--skip-restore",
        action="store_true",
        help="skips the restore process entirely, relying on existing DB",
    )
    parser.add_argument(
        "-d",
        "--disable-schema-changes",
        action="store_true",
        help="bypasses any column preparation that would affect schema definition",
    )
    parser.add_argument(
        "-l",
        "--leave-dump",
        action="store_true",
        help="do not delete dump file after anonymization",
    )
    parser.add_argument(
        "--schema",
        help="YAML config file with anonymization rules for all tables",
        required=True,
        default="./schema.yaml",
    )
    parser.add_argument(
        "-f",
        "--dump-file",
        help="path to the dump of DB to load and anonymize",
        default="to_anonymize.sql",
    )
    parser.add_argument("--dbname", help="name of the database to dump")
    parser.add_argument(
        "--user",
        help="name of the Postgres user with access to the anonymized database",
    )
    parser.add_argument(
        "--password",
