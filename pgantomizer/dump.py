...
def main():
    parser = argparse.ArgumentParser(
        description="Dump tables specified in YAML config file from production DB "
    ...
    parser.add_argument(
        "--schema",
        help="YAML config file with tables to dump",
        default="./schema.yaml",
        required=True,
    )
    parser.add_argument(
        "--dump-file",
        help="path to the file where to dump the DB",
        default="to_anonymize.sql",
    )
    parser.add_argument("--dbname", help="name of the database to dump")
    parser.add_argument(
        "--user", help="name of the Postgres user with access to the database"
    )
    parser.add_argument(
        "--password",
        help="password of the Postgres user with access to the database",
        default="",
    )
    parser.add_argument(
